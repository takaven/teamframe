...
Never use real ARIE, Baynunah, or customer data in launch-test environments. Approved TAKAVEN synthetic project refs are maintained in `scripts/approved-launch-projects.mjs` and their purposes/states in `docs/launch/environment-parity.md`; verify both before use. The older ARIE-associated project `qrsxoumymbcehtltbtgn` is **not** a test target.
