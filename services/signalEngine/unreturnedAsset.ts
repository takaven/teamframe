...
function isAssetReturnItem(row: ActionSourceRow): boolean {
  const haystack = `${row.category} ${row.title} ${row.suggested_action}`.toLowerCase();
  return (haystack.includes("asset") || haystack.includes("return")) && row.category !== "unreturned_asset";
}

export type UnreturnedAssetReconcileResult = {
...
export async function reconcileUnreturnedAssetSignals(params: {
  tenantId: string;
  actorUserId?: string;
  now?: Date;
}): Promise<UnreturnedAssetReconcileResult> {
  ...
  if (employeeError) {
    throw new Error(`UNRETURNED_ASSET_EMPLOYEE_QUERY_FAILED: ${employeeError.message}`);
  }
  ...
  if (actionSourceError) {
    throw new Error(`UNRETURNED_ASSET_ACTION_SOURCE_QUERY_FAILED: ${actionSourceError.message}`);
  }

  const actionSources = (actionSourceData ?? []) as ActionSourceRow[];
  const assetCountByEmployeeId = new Map<string, number>();
  for (const item of actionSources) {
    if (!item.subject_employee_id) continue;
    if (!isAssetReturnItem(item)) continue;
    assetCountByEmployeeId.set(item.subject_employee_id, (assetCountByEmployeeId.get(item.subject_employee_id) ?? 0) + 1);
  }
  ...
  const { data: openSignalData, error: openSignalError } = await supabase
    ...
    .eq("tenant_id", params.tenantId)
    .eq("kind", "unreturned_asset")
    .is("resolved_at", null);

  if (openSignalError) {
    throw new Error(`UNRETURNED_ASSET_SIGNAL_QUERY_FAILED: ${openSignalError.message}`);
  }
  ...
  for (const employee of employees) {
    const count = assetCountByEmployeeId.get(employee.id) ?? 0;
    if (count <= 0) continue;
  ...
  for (const [employeeId, entry] of desired.entries()) {
    ...
    const evidence = {
      trigger_reason: "unreturned_asset",
      subject_employee_id: employeeId,
      rule_version: "unreturned_asset_v1",
      open_asset_return_count: entry.count,
      what_is_wrong: `${entry.count} asset return item${entry.count === 1 ? " is" : "s are"} still open.`,
      why_it_matters: "Company property can be lost and access revocation can stay incomplete.",
      what_to_do_next: "Resolve the asset return items.",
    };
    ...
    if (!existing) {
      ...
      if (insertSignalError || !insertedSignal) {
        throw new Error(`UNRETURNED_ASSET_SIGNAL_CREATE_FAILED: ${insertSignalError?.message ?? "no row"}`);
      }
      ...
      const { error: actionError } = await supabase
        ...
        .insert({
          ...
          title: "Recover company asset",
          suggested_action: "Recover company asset",
          status: "open",
        } as never);

      if (actionError) {
        throw new Error(`UNRETURNED_ASSET_ACTION_CREATE_FAILED: ${actionError.message}`);
      }

      const { error: auditError } = await supabase.from("audit_logs").insert({
        tenant_id: params.tenantId,
        actor_user_id: params.actorUserId ?? "00000000-0000-0000-0000-000000000000",
        action_type: "signal.unreturned_asset.created",
        target_id: signalId,
      } as never);

      if (auditError) {
        throw new Error(`UNRETURNED_ASSET_AUDIT_CREATE_FAILED: ${auditError.message}`);
      }
    ...
    if (updateSignalError) {
      throw new Error(`UNRETURNED_ASSET_SIGNAL_UPDATE_FAILED: ${updateSignalError.message}`);
    }
  ...
  for (const signal of openSignals) {
    ...
    if (resolveSignalError) {
      throw new Error(`UNRETURNED_ASSET_SIGNAL_RESOLVE_FAILED: ${resolveSignalError.message}`);
    }
    ...
    if (actionFindError) {
      throw new Error(`UNRETURNED_ASSET_ACTION_QUERY_FAILED: ${actionFindError.message}`);
    }
    ...
    for (const item of (openActionItems ?? []) as ActionItemRow[]) {
      ...
      if (actionResolveError) {
        throw new Error(`UNRETURNED_ASSET_ACTION_RESOLVE_FAILED: ${actionResolveError.message}`);
      }
      ...
      if (linkSignalError) {
        throw new Error(`UNRETURNED_ASSET_SIGNAL_LINK_ACTION_FAILED: ${linkSignalError.message}`);
      }
    }
    ...
    const { error: auditError } = await supabase.from("audit_logs").insert({
      tenant_id: params.tenantId,
      actor_user_id: params.actorUserId ?? "00000000-0000-0000-0000-000000000000",
      action_type: "signal.unreturned_asset.resolved",
      target_id: signal.id,
    } as never);

    if (auditError) {
      throw new Error(`UNRETURNED_ASSET_AUDIT_RESOLVE_FAILED: ${auditError.message}`);
    }
