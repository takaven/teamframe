...
export async function GET(request: NextRequest) {
  ...
  const report = await getReports(actor, filters);
  let rows: Array<Record<string, string | number | null | undefined>> = [];
  if (reportName === "headcount") rows = report.active.map((e) => ({ employee: e.full_name, department: e.department, location: e.work_location, employment_type: e.employment_type, country: e.country, start_date: e.start_date }));
  ...
  if (reportName === "time-off" || reportName === "away") rows = (reportName === "away" ? report.away : report.periodLeaves).map((l) => ({ employee: report.names.get(l.employee_id), leave_type: l.leave_type, start_date: l.start_date, end_date: l.end_date, days: l.requested_days, status: l.status }));
  ...
  if (reportName === "onboarding") rows = report.onboardingRows.map((r) => ({ employee: r.employee.full_name, tasks: r.tasks, completed: r.complete, completion_percent: r.tasks ? Math.round(r.complete / r.tasks * 100) : null, overdue: r.overdue, check_ins: r.checkIns.map((c) => c.status).join(";"), probation: r.probation.map((p) => p.outcome ?? p.status).join(";") }));
