...
export async function GET(request: NextRequest) {
  ...
  if (reportName === "time-off" || reportName === "away") rows = (reportName === "away" ? report.away : report.periodLeaves).map((l) => ({ employee: report.names.get(l.employee_id), leave_type: humanCsvValue(l.leave_type), start_date: l.start_date, end_date: l.end_date, days: l.requested_days, status: humanCsvValue(l.status) }));
