...
export default async function PeopleImportPage({ searchParams }: { searchParams: Promise<{ error?: string; created?: string }> }) {
  ...
  return <main className="mx-auto max-w-5xl px-6 py-14">
    ...
    <header className="mt-4 border-b border-ink-200 pb-5">
      ...
      <p className="mt-1 text-[14px] text-ink-500">Paste a CSV, review every row, then confirm the import into this workspace only. Use an existing or earlier-row work email for the manager.</p>
    </header>
    ...
    <section className="mt-6 tf-surface-flat p-5">
      <a download="teamframe-people-template.csv" href={`data:text/csv;charset=utf-8,${encodeURIComponent("full_name,email,employee_number,role_title,department,manager_email,start_date,employment_type,country,work_location\n")}`} className="tf-secondary-action px-3 py-2 text-[12px]">Download template</a>
      <div className="mt-5"><ImportForm /></div>
    </section>
  </main>;
}
