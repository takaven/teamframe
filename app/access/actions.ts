...
const MatrixSchema = z.object({
  ...
  people_selected_employee_ids: z.array(z.string().uuid()).default([]),
  ...
  salary_selected_employee_ids: z.array(z.string().uuid()).default([]),
  private_documents_scope: z.enum(["none", "all", "selected_people", "all_except_selected_people"]),
  private_documents_selected_employee_ids: z.array(z.string().uuid()).default([]),
...
export async function updateAccessMatrixAction(formData: FormData): Promise<void> {
  try {
    ...
    const parsed = MatrixSchema.parse({
      ...
      people_selected_employee_ids: formData.getAll("people_selected_employee_ids"),
      salary_access_level: formData.get("salary_access_level"),
      salary_access_scope: formData.get("salary_access_scope"),
      salary_selected_employee_ids: formData.getAll("salary_selected_employee_ids"),
      private_documents_scope: formData.get("private_documents_scope"),
      private_documents_selected_employee_ids: formData.getAll("private_documents_selected_employee_ids"),
    ...
    await updateAccessMatrix(actor, {
      ...
      matrix: {
        ...
        peopleSelectedEmployeeIds: parsed.people_selected_employee_ids,
        salaryAccessLevel: parsed.salary_access_level,
        salaryAccessScope: parsed.salary_access_scope,
        salarySelectedEmployeeIds: parsed.salary_selected_employee_ids,
        privateDocumentsScope: parsed.private_documents_scope,
        privateDocumentsSelectedEmployeeIds: parsed.private_documents_selected_employee_ids,
