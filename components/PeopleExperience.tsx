...
import { CopyInviteEmailButton } from "@/app/employees/CopyInviteEmailButton";
import {
...
type PeopleView = "directory" | "record" | "create";

export async function PeopleExperience({
  searchParams,
  view = "directory",
  employeeId,
}: {
  ...
  const { status, error, employee: queryEmployee, activation_link: activationLink, q, filter, tab } = await searchParams;
  const employeeParam = employeeId ?? queryEmployee;
  ...
  const detailEmployees = employeeParam
    ...
    : [];
  if (view === "record" && detailEmployees.length === 0) notFound();
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-14">
      ...
      <section className="tf-surface overflow-hidden">
        ...
        {employees.length === 0 ? (
          <EmptyState
            ...
            cta={{ label: "Add person", href: "/people/add" }}
            className="m-5"
          />
      ...
      {view === "record" ? <section id="overview" className="space-y-4 scroll-mt-6">
        {employees.length === 0 ? (
          null
            ...
            <article id={`employee-${employee.id}`} key={employee.id} className="tf-print-record space-y-4">
              ...
              <SectionTabs
                ...
                tabs={[
                  ...
                  { id: "onboarding-offboarding", label: "Onboarding & offboarding" },
                  { id: "compensation-payment", label: "Compensation & payment" },
              ...
              >
                ...
                <div data-tab="overview">
                  <StarterReadinessPanel
                    ...
                    accountState={employee.activated_at ? "Activated" : employee.invite_last_sent_at ? "Invite sent" : employee.invite_attempt_count > 0 ? "Invite delivery pending" : "Not invited"}
                    blockers={readiness.blockers}
                  />
                </div>
                <section data-tab="overview" className="rounded-xl border border-ink-200 bg-white/70 p-5">
                  <h3 className="text-[14px] font-semibold text-ink-900">Needs attention</h3>
                  <div className="mt-3 divide-y divide-ink-100 border-t border-ink-100">
                    ...
                    {(onboardingByEmployee.get(employee.id) ?? []).filter((task) => task.status !== "completed").slice(0, 3).map((task) => (
                      <a key={task.id} href="#onboarding-offboarding" className="flex items-center justify-between gap-3 py-3 text-[12px] hover:text-ink-900">
                        <span>{task.title}</span><span className="font-semibold">Open task</span>
                      </a>
                    ))}
                ...
                <div data-tab="history" className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-ink-200 bg-white/70 p-5">
                  <div>
              ...
              <form action={updateEmployeeAction} data-tab="legacy-direct-edit" className="grid gap-3 rounded-xl border border-ink-200 bg-white/70 p-5 md:grid-cols-4">
              ...
              <section data-tab="employment" className="rounded-xl border border-ink-200 bg-white/70 p-5">
                ...
                <form action={recordEmploymentChangeAction} className="mt-3 grid gap-2 md:grid-cols-3">
                  ...
                  <label className="flex flex-col gap-1 text-[11px] text-ink-500">
                    Start date
                    <DateField name="start_date" dense />
                  </label>
                  <label className="flex flex-col gap-1 text-[11px] text-ink-500">
                    End date
                    <DateField name="end_date" dense />
                  </label>
                  <label className="flex flex-col gap-1 text-[11px] text-ink-500">
                    Manager
                    <select
              ...
              <section data-tab="documents" className="tf-surface-flat flex flex-col p-5">
                ...
                <div className="order-last mt-4 rounded-lg border border-ink-100 bg-ink-50/40 p-4">
                  <h5 className="text-[13px] font-semibold text-ink-900">Request or upload a document</h5>
